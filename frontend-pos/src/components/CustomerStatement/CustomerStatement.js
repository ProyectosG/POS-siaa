...
import { DollarSign, CreditCard, Search, Package, ArrowUpRight, ArrowDownLeft } from "lucide-react"
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
export default function CustomerStatement() {
  ...
  const [balanceHistory, setBalanceHistory] = useState([]) // ← Nuevo estado para historial
  ...
  // Cargar estado de cuenta + historial de balance cuando se selecciona cliente
  useEffect(() => {
    ...
    const fetchData = async () => {
      setLoading(true)
      try {
        // 1. Ventas / estado de cuenta
        const salesRes = await fetch(`${API_URL}/customers/${selectedCustomerId}/statement`)
        if (!salesRes.ok) throw new Error("Error al cargar ventas")
        const salesData = await salesRes.json()
        setVentas(salesData)

        // 2. Historial de balance
        const historyRes = await fetch(`${API_URL}/customers/${selectedCustomerId}/balance-history`)
        if (!historyRes.ok) throw new Error("Error al cargar historial de balance")
    ...
    <div className="space-y-6 p-4 md:p-6 lg:p-8 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl md:text-3xl font-bold text-gray-900">Estado de Cuenta</h1>
            <p className="text-sm text-gray-600">Historial completo, pagos y saldos pendientes</p>
          </div>
        </div>
      </div>

      {/* Panel superior: Selector + Tarjetas resumen */}
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 lg:gap-6">
        ...
        {selectedCustomerId && (
          <>
            <Card className="border-blue-100 bg-gradient-to-br from-blue-50/80 to-white hover:shadow-md transition-shadow">
              <CardContent className="p-4">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-xs font-medium text-blue-700">Total Vendido</p>
            ...
            <Card className="border-green-100 bg-gradient-to-br from-green-50/80 to-white hover:shadow-md transition-shadow">
              <CardContent className="p-4">
                <div className="flex items-center justify-between">
                  <div>
            ...
            <Card className="border-red-100 bg-gradient-to-br from-red-50/80 to-white hover:shadow-md transition-shadow">
              <CardContent className="p-4">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-xs font-medium text-red-700">Saldo Pendiente</p>
      ...
      {selectedCustomerId ? (
        <div className="space-y-8">
          ...
          <Card className="border-gray-200 shadow-sm">
            <CardContent className="p-5">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
                <Input 
                  placeholder="Buscar folio o razón..." 
                  value={filtro} 
                ...
                <Input type="date" value={filtroFechaInicio} onChange={e => setFiltroFechaInicio(e.target.value)} className="h-10" />
                <Input type="date" value={filtroFechaFin} onChange={e => setFiltroFechaFin(e.target.value)} className="h-10" />
              </div>
            </CardContent>
          </Card>
                  ...
                  <TableBody>
                    {loading ? (
                      ...
                      ventasFiltradas.map(v => (
                        <TableRow 
                          key={v.id} 
                          className="hover:bg-indigo-50/30 transition-colors border-b last:border-0"
                        >
          ...
          {/* Nueva sección: Historial de Movimientos del Cliente */}
          <Card className="border-gray-200 shadow-sm overflow-hidden">
            <CardContent className="p-5">
              <h2 className="text-xl font-bold mb-4 flex items-center gap-2">
                <ArrowUpRight className="h-5 w-5 text-indigo-600" />
                Historial Completo de Movimientos (Cronológico)
              </h2>
                  ...
                  <Table>
                    <TableHeader className="bg-gray-50">
                      <TableRow>
                        ...
                        <TableHead className="text-right w-32 font-semibold">Saldo Actual</TableHead>
                        <TableHead>Descripción</TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                      {balanceHistory.map(m => (
                        <TableRow 
                          key={m.id}
                          className="hover:bg-gray-50 transition-colors border-b last:border-0"
                        >
                          <TableCell className="text-sm">
                            {m.date} <span className="text-gray-500">{m.time}</span>
                          </TableCell>
                          <TableCell>
                            <Badge 
                              variant="outline"
                              className={m.amount < 0 
                                ? "bg-green-100 text-green-800 border-green-300" 
                                : "bg-red-100 text-red-800 border-red-300"}
                            >
                              {m.amount < 0 ? (
                                <div className="flex items-center gap-1">
                                  <ArrowDownLeft className="h-4 w-4" /> Abono
                                </div>
                              ) : (
                                <div className="flex items-center gap-1">
                                  <ArrowUpRight className="h-4 w-4" /> Venta/Cargo
                                </div>
                              )}
                            </Badge>
                          </TableCell>
                          <TableCell className="font-medium">{m.sale_id || "—"}</TableCell>
                          <TableCell className="text-right font-medium">
                            ${Number(m.previous_balance || 0).toFixed(2)}
                          </TableCell>
                          <TableCell className="text-right font-bold" style={{
                            color: m.amount < 0 ? '#16a34a' : '#dc2626'
                          }}>
                            {m.amount < 0 ? '' : '+'}${Math.abs(Number(m.amount)).toFixed(2)}
                          </TableCell>
                          <TableCell className="text-right font-bold">
                            ${Number(m.new_balance || 0).toFixed(2)}
                          </TableCell>
        ...
        <Card className="border-dashed border-2 border-gray-300 bg-gray-50/50">
          <CardContent className="p-12 text-center">
            ...
            <p className="text-gray-500">
              Elige un cliente de la lista superior para ver su historial completo de ventas, pagos y movimientos.
            </p>
          </CardContent>
        </Card>
      )}
